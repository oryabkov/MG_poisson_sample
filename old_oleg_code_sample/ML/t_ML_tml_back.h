#ifndef __T_ML_TML_H__
#define __T_ML_TML_H__
...
class t_ML_tml
{
	...
	t_ML_tml	&operator=(const t_ML_tml& x)
	{
	}
	t_ML_tml(const t_ML_tml& x)
	{
	}
public:
	t_ML_tml()
	...
	{
		printf("t_ML_tml::operator()\n");
		int levs_num = smoother.size();
		for (int i = 1;i < levs_num;++i) {
			printf("t_ML_tml::operator(): proj x to level %d\n", i);
			if (i == 1) (*proj[i])(x,*bi[i]); else (*proj[i])(*bi[i-1],*bi[i]);
		}
		int	ilev_prev;
		for (int i = 0;i < hierarchy.size();++i) {
			int ilev = hierarchy[i];
			printf("t_ML_tml::operator(): do hierarchy %d on level %d\n", i, ilev);
			assert(ilev > 0);
			if (i == 0) {
			...
			printf("t_ML_tml::operator(): call smoother on level %d\n", ilev);
